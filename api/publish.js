export default async function handler(req, res) {
  ...
  const { posts, title } = req.body;
  if (!posts) {
    return res.status(400).json({ message: 'Missing posts database payload.' });
  }
  ...
  try {
    ...
    const fileContentString = JSON.stringify(posts, null, 2);
    const base64Content = Buffer.from(fileContentString, 'utf-8').toString('base64');

    const putBody = {
      message: `content: publish blog article "${title || 'Untitled'}" via Admin Portal`,
